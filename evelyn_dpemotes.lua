--[[
    ...
    converted by evelyn#1571
]]
local DP = {}

-- DP.Expressions = { --no native supported this yet as for now on 2take1
...
--   ["Trevor"] = {"move_p_m_two"},
--   ["Wide"] = {"move_m@bag"},
-- -- -- I cant get these below to work for some reason, if anyone knows a fix lmk 
--     --[[
...
local function SendNotif(msg)
    menu.notify(msg, "Evelyn DP-Emotes", 10)
end
    ...
    for i=1, #PlayerProps do
        ...
        if entity.is_an_entity(ent) then
            if network.has_control_of_entity(ent) then
                ...
                for tries = 0, 100 do 
                    if not entity.is_an_entity(ent) then break end
...
local function AddPropToPlayer(propmodel, bone, off1, off2, off3, rot1, rot2, rot3)
    local pped = player.get_player_ped(-1)
    local pcoords = entity.get_entity_coords(pped) + v3(0.0, 0.0, 0.2)
    propmodel = tonumber(propmodel) or gameplay.get_hash_key(propmodel)
    ...
    local prop = object.create_object(propmodel, pcoords,  true,  true)
    entity.attach_entity_to_entity(prop, pped, ped.get_ped_bone_index(pped, bone), v3(off1 or 0.0, off2 or 0.0, off3 or 0.0), v3(rot1 or 0.0, rot2 or 0.0, rot3 or 0.0), false, false, false, 1, true)
...
-- local function PtfxStart()
    ...
    -- Ptfx = StartNetworkedParticleFxLoopedOnEntityBone(PtfxName, pped, Ptfx1, Ptfx2, Ptfx3, Ptfx4, Ptfx5, Ptfx6, GetEntityBoneIndexByName(PtfxName, "VFX"), 1065353216, 0, 0, 0, 1065353216, 1065353216, 1065353216, 0)
...
local function CheckGender()
    local hash = entity.get_entity_model_hash(player.get_player_ped(-1))
...
local function OnEmotePlay(EmoteName)
    local InVehicle = ped.is_ped_in_any_vehicle(player.get_player_ped(-1), true)
    -- if not Config.AllowedInCars and InVehicle == 1 then
    --   return
    -- end
  
    if not entity.is_an_entity(player.get_player_ped(-1)) then
      return false
    end
  
    -- if Config.DisarmPlayer then
    --     SetCurrentPedWeapon(player.get_player_ped(-1), gameplay.get_hash_key('WEAPON_UNARMED'), true)
    -- end
    ...
    --   SetFacialIdleAnimOverride(player.get_player_ped(-1), ChosenAnimation, 0) --2take1 dont have this native yet
    --   return
    -- end
    ...
    if ChosenDict == "MaleScenario" or "Scenario" then 
      ...
      if ChosenDict == "MaleScenario" then if InVehicle then return end
        if PlayerGender == "male" then
          ped.clear_ped_tasks_immediately(pped)
          ai.task_start_scenario_in_place(pped, ChosenAnimation, 0, true)
          print("Playing scenario = ("..ChosenAnimation..")")
          IsInAnimation = true
        else
          SendNotif("This emote is for male only")
        end return
      elseif ChosenDict == "ScenarioObject" then if InVehicle then return end
        --local BehindPlayer = GetOffsetFromEntityInWorldCoords(pped, 0.0, 0 - 0.5, -0.5); --2take1 dont have this native yet (replaced with playercoords for now)
        ped.clear_ped_tasks_immediately(pped)
        ai.task_start_scenario_at_position(pped, ChosenAnimation, entity.get_entity_coords(pped) - v3(0.0, 0.0, 0.5), entity.get_entity_heading(pped), 0, true, false)
        print("Playing scenario = ("..ChosenAnimation..")")
        IsInAnimation = true
        return
    ...
    ai.task_play_anim(player.get_player_ped(-1), ChosenDict, ChosenAnimation, 2.0, 2.0, AnimationDuration, MovementType, 0, false, false, false)
...
local m_ = menu.add_feature("evelyn dpemotes", "parent", 0, function() end).id
menu.add_feature("cancel emote by pressing X", "toggle", m_, function(f)
    if f.on then 
        if controls.is_control_pressed(0, 73) then 
            EmoteCancel()
        end
...
local function playDance(n)
    ...
    if not n or type(n) ~= "number" then
        ...
        SendNotif("You randomly selected dance #"..n.."\nYou can play this dance again using /dance "..n)
    end

    if n > dance_ceil or n <= 0 then
        SendNotif("There are only "..dance_ceil.." dances, select a number inbetween or random (no input)")
        return
    end 

    if e_dances[n] and e_dances[n].disabled then
        SendNotif("Dance "..n.." is disabled")
        return
    end

    LoadAnim(e_dances[n].dict)
    ai.task_play_anim(player.get_player_ped(-1), e_dances[n].dict, e_dances[n].anim, 3.0, 3.0, -1, 1, 0, false, false, false)
    IsInAnimation = true
    streaming.remove_anim_dict(e_dances[n].dict)
end

local m_parent = {
    emotes = menu.add_feature("emote list", "parent", m_, function() end).id,
    emotesProps = menu.add_feature("emote with prop list", "parent", m_, function() end).id,
    dances = menu.add_feature("dance list", "parent", m_, function() end).id,
    randomDance = menu.add_feature("random dance", "action", m_, playDance).id
}
...
for k,_ in spairs(DP.Emotes, function(t, a, b) return t[a][3] < t[b][3] end) do 
    menu.add_feature(k, "action", m_parent.emotes, function() return OnEmotePlay(_) end)
end
for k,_ in spairs(DP.PropEmotes, function(t, a, b) return t[a][3] < t[b][3] end) do 
    menu.add_feature(k, "action", m_parent.emotesProps, function() return OnEmotePlay(_) end)
end
for k,_ in spairs(DP.Dances, function(t, a, b) return t[a][3] < t[b][3] end) do 
    menu.add_feature(k, "action", m_parent.dances, function() return OnEmotePlay(_) end)
end



event.add_event_listener("chat", function(event)
    if event.player ~= player.player_id() then return end
    if event.body:find("/e ") then 
        local name = event.body:gsub("/e ", "")
        if name == "c" or name == "cancel" then 
            return EmoteCancel()
        end

        if DP.Emotes[name] ~= nil then
            return OnEmotePlay(DP.Emotes[name]) 
        elseif DP.Dances[name] ~= nil then
            return OnEmotePlay(DP.Dances[name]) 
        elseif DP.PropEmotes[name] ~= nil then
            return OnEmotePlay(DP.PropEmotes[name])
        else
            return SendNotif("'"..name.."' is not a valid emote!")
        end
    elseif event.body:find("/dance") then 
        playDance(tonumber((event.body:gsub("/dance ", ""))))
    end
end)
